...
class AdvancedBudgetTracker {
  ...
  updateSummaryCards() {
    ...
    this.animateValue("totalIncome", totalIncome, "$")
    this.animateValue("totalExpenses", totalExpenses, "$")
    this.animateValue("netBalance", netBalance, "$")
      ...
      .map((transaction) => {
        ...
        return `
                <div class="transaction-item ${transaction.type} flex items-center justify-between p-3 rounded-lg ${bgClass}">
                    ...
                    <span class="${colorClass} font-bold">$${transaction.amount.toFixed(2)}</span>
                </div>
            `
      })
      .join("")
  ...
  updateBudgetProgress() {
    ...
    if (this.budget > 0) {
      ...
      document.getElementById("budgetSpent").textContent = `$${totalExpenses.toFixed(2)} spent`
  ...
  exportToCSV() {
    ...
    const csvContent = [
      headers.join(","),
      ...this.transactions.map((t) => [t.date, t.type, t.category, `"${t.description || ""}"`, t.amount].join(",")),
    ].join("\n")
